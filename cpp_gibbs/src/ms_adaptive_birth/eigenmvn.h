...
namespace Eigen {
    ...
    */
    ...
    public:
        EigenMultivariateNormal(const bool use_cholesky = false) : _use_cholesky(use_cholesky) {
        ...
        void setParams(const Matrix<Scalar, Dynamic, 1> &mean, const Matrix<Scalar, Dynamic, Dynamic> &covar) {
            setMean(mean);
            setCovar(covar);
        }
        ...
        void setCovar(const Matrix<Scalar, Dynamic, Dynamic> &covar) {
            ...
            if (_use_cholesky) {
                ...
                _transform =
                        _eigenSolver.eigenvectors() * _eigenSolver.eigenvalues().cwiseMax(0).cwiseSqrt().asDiagonal();
            }
        }
