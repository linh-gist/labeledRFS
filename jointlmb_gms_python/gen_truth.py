...
class truth:
    def __init__(self, model):
        ...
        self.track_list = {key: np.empty(0, dtype=int) for key in range(0, self.K)};  # absolute index target identities (plotting)
        ...
        xstart[:, 0:1] = np.array([[0], [0], [0], [-10]]);
        tbirth[0] = 1;
        tdeath[0] = 70;
        xstart[:, 1:2] = np.array([[400], [-10], [-600], [5]]);
        tbirth[1] = 1;
        tdeath[1] = self.K + 1;
        xstart[:, 2:3] = np.array([[-800], [20], [-200], [-5]]);
        tbirth[2] = 1;
        tdeath[2] = 70;

        xstart[:, 3:4] = np.array([[400], [-7], [-600], [-4]]);
        tbirth[3] = 20;
        tdeath[3] = self.K + 1;
        xstart[:, 4:5] = np.array([[400], [-2.5], [-600], [10]]);
        tbirth[4] = 20;
        tdeath[4] = self.K + 1;
        xstart[:, 5:6] = np.array([[0], [7.5], [0], [-5]]);
        tbirth[5] = 20;
        tdeath[5] = self.K + 1;

        xstart[:, 6:7] = np.array([[-800], [12], [-200], [7]]);
        tbirth[6] = 40;
        tdeath[6] = self.K + 1;
        xstart[:, 7:8] = np.array([[-200], [15], [800], [-10]]);
        tbirth[7] = 40;
        tdeath[7] = self.K + 1;

        xstart[:, 8:9] = np.array([[-800], [3], [-200], [15]]);
        tbirth[8] = 60;
        tdeath[8] = self.K + 1;
        xstart[:, 9:10] = np.array([[-200], [-3], [800], [-15]]);
        tbirth[9] = 60;
        tdeath[9] = self.K + 1;

        xstart[:, 10:11] = np.array([[0], [-20], [0], [-15]])
        tbirth[10] = 80
        tdeath[10] = self.K + 1
        xstart[:, 11:12] = np.array([[-200], [15], [800], [-5]])
        tbirth[11] = 80
        tdeath[11] = self.K + 1
