...
def plot_results(model, truth, meas, est):
    ...
    for t in range(Y_track.shape[2]):
        plt_x_est = axs1.scatter(np.arange(0, truth.K), Y_track[0, :, t], marker='.', s=80, color=colorarray.rgb[:, t], label='Estimates')
    axs1.legend(handles=[plt_x_meas, plt_x_truth[0], plt_x_est])
    ...
    for i in range(0, truth.total_tracks):
        ...
        plt_y_truth = axs2.plot(np.arange(k_birth[i], k_death[i], 1), Py[1, :], linestyle='-', linewidth=1,
                 color=0 * np.ones((1, 3)), label='True tracks')
    # plot y estimate
    for t in range(0, Y_track.shape[2]):
        plt_y_est = axs2.scatter(np.arange(0, truth.K), Y_track[2, :, t], marker='.', s=80, color=colorarray.rgb[:, t], label='Estimates')
    # plt.legend('Estimates')
    axs2.set_xlabel('Time');
    axs2.set_ylabel('y-coordinate (m)')
    axs2.legend(handles=[plt_y_meas, plt_y_truth[0], plt_y_est])
    plt.show()
